...
public class AddNewAuthorCommand {
    ...
    public static int run(CommandContext<ServerCommandSource> context, String playerName){
        if (!ModConfig.getBooleanValue("authorsCanAddAuthors") ){
            context.getSource().sendFeedback(() -> Text.literal("Command is disabled in the mod config"), false);
            return -1;
        }
        ...
        if(!itemStack.isOf(Items.FILLED_MAP)){
            context.getSource().sendFeedback(() -> Text.literal("You should have filled map in main hand"), false);
            return -1;
        } else if (!canCopy(itemStack, player)) {
            context.getSource().sendFeedback(() -> Text.literal("You`re not one of the authors, who are allowed to modify this map"), false);
            return -1;
        }

        if(!addAuthor(itemStack, playerName)){
            context.getSource().sendFeedback(() -> Text.literal("There is already the author"), false);
            return -1;
        }
